...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        $products = Product::query()
            ->with(['translations', 'primaryVariant', 'images', 'category.translations'])
            ->where('status', true)
            ->when($request->filled('q'), function ($query) use ($request) {
                ...
                $query->where(function ($query) use ($term) {
                    $query->where('slug', 'like', "%{$term}%")
                        ...
                        ->orWhereHas('variants', fn ($variant) => $variant->where('SKU', 'like', "%{$term}%"));
                });
            })
        ...
        $categories = \App\Models\Category::with(['translations'])
        ...
        return view('themes.xylo.product', compact('products', 'categories'));
    }
