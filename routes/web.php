...
// Pages route (must be last to avoid conflicts with other routes)
Route::get('/{slug}', [App\Http\Controllers\Store\PageController::class, 'show'])->name('page.show')
    ->where('slug', '^(?!admin|login|register|password|cart|checkout|customer|stripe|product|category|search|products|inquiries|change-currency|change-store-language|site-settings|document).+$');
